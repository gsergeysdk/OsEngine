...
{
    public class TrandPhase : Aindicator
    {
        ...
        public override void OnStateChange(IndicatorState state)
        {
            ...
            {
                _length_s = CreateParameterInt("Period fast", 200);
                _length_b = CreateParameterInt("Period slow", 500);
                _trand_counter = CreateParameterInt("Trand counter", 10);
                ...
                _ma_s = IndicatorsFactory.CreateIndicatorByName("Ssma", Name + "Ssma", false);
                ((IndicatorParameterInt)_ma_s.Parameters[0]).Bind(_length_s);
                ProcessIndicator("MA_S", _ma_s);
                _ma_b = IndicatorsFactory.CreateIndicatorByName("Ssma", Name + "Ssma", false);
                ((IndicatorParameterInt)_ma_b.Parameters[0]).Bind(_length_b);
                ProcessIndicator("MA_B", _ma_b);
            }
        }

        public override void OnProcess(List<Candle> candles, int index)
        {
            if (index < _length_s.ValueInt || index < _length_b.ValueInt)
            {
