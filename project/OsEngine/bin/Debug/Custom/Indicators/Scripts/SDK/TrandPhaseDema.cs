...
{
    public class TrandPhaseDema : Aindicator
    {
        ...
        public override void OnStateChange(IndicatorState state)
        {
            if (state == IndicatorState.Configure)
            {
                ...
                _ma_s = IndicatorsFactory.CreateIndicatorByName("DEMA_ssma", Name + "Ema", false);
                ((IndicatorParameterInt)_ma_s.Parameters[0]).Bind(_length_s);
                ProcessIndicator("DEMA_S", _ma_s);
                _ma_b = IndicatorsFactory.CreateIndicatorByName("DEMA_ssma", Name + "Ema", false);
                ((IndicatorParameterInt)_ma_b.Parameters[0]).Bind(_length_b);
                ProcessIndicator("DEMA_B", _ma_b);
            }
        }
