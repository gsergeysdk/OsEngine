...
{
    [Bot("SDKThreeSoldierAdaptive")]
    public class SDKThreeSoldierAdaptive : BotPanel
    {
        public SDKThreeSoldierAdaptive(string name, StartProgram startProgram)
            : base(name, startProgram)
        {
            TabCreate(BotTabType.Simple);
            _tabScreener = TabsSimple[0];
            ...
            BreakevenStopPercent = CreateParameter("Break even %", 20m, 0, 20, 1m);
        ...
        private BotTabSimple _tabScreener;
        ...
        public StrategyParameterDecimal BreakevenStopPercent;
        ...
        {
            BotTabSimple tab = _tabScreener;
            SecuritiesTradeSettings mySettings = null;

            for(int i = 0;i < _tradeSettings.Count;i++)
            {
                if (_tradeSettings[i].SecName == tab.Securiti.Name &&
                    _tradeSettings[i].SecClass == tab.Securiti.NameClass)
                {
        ...
        private void LogicClosePosition(List<Candle> candles, BotTabSimple tab, SecuritiesTradeSettings settings)
        {
            ...
            for (int i = 0; openPositions != null && i < openPositions.Count; i++)
            {
                ...
                if (position.Direction == Side.Buy)
                {
                    ...
                    {
                        if (BreakevenStopPercent.ValueDecimal > 0)
                        {
                            decimal priceTrailing = _lastPrice - (_lastPrice * BreakevenStopPercent.ValueDecimal) / 100;
                            if (priceTrailing > position.EntryPrice && position.StopOrderPrice < position.EntryPrice && priceTrailing >= position.EntryPrice)
                                tab.CloseAtTrailingStop(position, priceTrailing, priceTrailing - priceTrailing * (Slippage.ValueDecimal / 100));
                        }
                        continue;
                    }
                    tab.CloseAtStop(position, priceStop, priceStop - priceStop * (Slippage.ValueDecimal / 100));
                    tab.CloseAtProfit(position, priceTake, priceTake - priceStop * (Slippage.ValueDecimal / 100));
                }
                else
                {
        ...
        private decimal GetVolume(BotTabSimple tab)
        {
            ...
            else if (VolumeType.ValueString == "Deposit percent")
            {
                ...
                if (tab.StartProgram == StartProgram.IsOsTrader || tab.StartProgram == StartProgram.IsTester)
                {
