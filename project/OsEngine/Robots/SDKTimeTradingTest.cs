...
{
    [Bot("SDKTimeTradingTest")]
    public class SDKTimeTradingTest : BotPanel
    {
        BotTabScreener _tab;
        ...
        private StrategyParameterInt ssmaFilterLen;
        private StrategyParameterBool useSsmaFilter;
        ...
        public SDKTimeTradingTest(string name, StartProgram startProgram) : base(name, startProgram)
        {
            ...
            ssmaFilterLen = CreateParameter("Ssma period", 10, 10, 50, 10);
            useSsmaFilter = CreateParameter("Use Ssma Filter", true);
            ...
            _tab.CreateCandleIndicator(3, "Ssma", new List<string>() { ssmaFilterLen.ValueInt.ToString() }, "Prime");
        ...
        private void LogicCheckIndicators(BotTabSimple tab)
        {
            ...
            Aindicator ssma = (Aindicator)tab.Indicators[2];

            if (ssma.ParametersDigit[0].Value != ssmaFilterLen.ValueInt)
            {
                ssma.ParametersDigit[0].Value = ssmaFilterLen.ValueInt;
                ssma.Save();
                ssma.Reload();
            }
        }
        ...
        private Side hasSignalToOpen(List<Candle> candles, BotTabSimple tab)
        {
            ...
            decimal lastSsma = ssma.DataSeries[0].Last;
            decimal prevSsma = ssma.DataSeries[0].Values[^2];

            if (useSsmaFilter.ValueBool && (lastSsma <= prevSsma))
                return Side.None;
